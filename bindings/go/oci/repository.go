...
// AddLocalResource adds a local resource to the repository. When the resource
// has local relation and is an OCI-compliant manifest, an ownership referrer is
// pushed alongside it (ADR 0016).
...
func (repo *Repository) uploadAndUpdateLocalArtifact(
	...
	configureOwnershipReferrer(&packOptions, artifact, component, version)
...
//   - Create a new referrer (ocm add cv). Otherwise, for a resource this
//     component owns (relation "local"), Create builds a fresh referrer for the
//     uploaded manifest.
//
...
// resources, so they get neither.
func configureOwnershipReferrer(packOptions *pack.Options, artifact descriptor.Artifact, component, version string) {
	res, isResource := artifact.(*descriptor.Resource)
	if !isResource {
		return
	}

	// Copy an inbound ownership referrer; otherwise create one for a resource this
	// component owns.
	src := tar.ReferrerSource{ArtifactType: annotations.OwnershipArtifactType}
	if res.Relation == descriptor.LocalRelation {
...
func (repo *Repository) UploadResource(ctx context.Context, res *descriptor.Resource, b blob.ReadOnlyBlob) (newRes *descriptor.Resource, err error) {
	...
	desc, access, err := repo.uploadOCIImage(ctx, res.Access, b, res.Relation == descriptor.LocalRelation)
	if err != nil {
...
// only the referrer manifests are added. A locally-owned artifact that carries
// no referrer is logged as a warning and is otherwise a no-op.
func (repo *Repository) uploadOwnershipReferrers(ctx context.Context, src content.ReadOnlyStorage, dst content.Storage, main ociImageSpecV1.Descriptor, referrers []ociImageSpecV1.Descriptor) error {
	if len(referrers) == 0 {
		slogcontext.Log(ctx, slog.LevelWarn, "resource has local relation but no ownership referrer was found in the artifact; nothing to copy", log.DescriptorLogAttr(main))
		return nil
	}
...
// attached to a resource kept by reference as an OCI image. It resolves the
// referenced image in its hosting registry, lists the referrers of that subject
// filtered by the ownership artifact type, and reads each referrer's annotations
// into a [repository.OwnershipReferrer]. It returns an empty slice when the image
// carries no ownership referrer (or when the store cannot answer referrer queries).
func (repo *Repository) GetOwnershipReferrer(ctx context.Context, resource *descriptor.Resource) (_ []repository.OwnershipReferrer, err error) {
	ctx = slogcontext.NewCtx(ctx, repo.logger)
	done := log.Operation(ctx, "get ownership referrer", log.IdentityLogAttr("resource", resource.ToIdentity()))
	defer func() {
		done(err)
	}()
	...
	var access accessv1.OCIImage
	if err := repo.scheme.Convert(resource.Access, &access); err != nil {
		return nil, fmt.Errorf("error converting resource access to OCI image for ownership referrer: %w", err)
	}
	...
	ref, err := looseref.ParseReference(access.ImageReference)
	if err != nil {
		return nil, fmt.Errorf("failed to parse image reference %q: %w", access.ImageReference, err)
	}
	subject, err := store.Resolve(ctx, ref.ReferenceOrTag())
	if err != nil {
		return nil, fmt.Errorf("failed to resolve subject %q for ownership referrer: %w", access.ImageReference, err)
	}

	referrers, err := lookupOwnershipReferrers(ctx, store, subject)
	if err != nil {
		return nil, fmt.Errorf("failed to list ownership referrers of %q: %w", access.ImageReference, err)
	}

	owners := make([]repository.OwnershipReferrer, 0, len(referrers))
	for _, referrer := range referrers {
		owner, err := readOwnershipReferrer(ctx, store, referrer)
		if err != nil {
			return nil, err
		}
...
// readOwnershipReferrer fetches an ownership referrer manifest and reads its
// ADR 0016 annotations into a [repository.OwnershipReferrer].
func readOwnershipReferrer(ctx context.Context, store content.Fetcher, desc ociImageSpecV1.Descriptor) (repository.OwnershipReferrer, error) {
	raw, err := content.FetchAll(ctx, store, desc)
	if err != nil {
		return repository.OwnershipReferrer{}, fmt.Errorf("failed to fetch ownership referrer %s: %w", desc.Digest, err)
	}
	var manifest ociImageSpecV1.Manifest
	if err := json.Unmarshal(raw, &manifest); err != nil {
		return repository.OwnershipReferrer{}, fmt.Errorf("failed to unmarshal ownership referrer %s: %w", desc.Digest, err)
	}
	owner := repository.OwnershipReferrer{
		Component: manifest.Annotations[annotations.OwnershipComponentName],
		Version:   manifest.Annotations[annotations.OwnershipComponentVersion],
	}
	// software.ocm.artifact carries the owned artifact's identity and kind; take
	// the first identity when present.
	if artifactJSON := manifest.Annotations[annotations.ArtifactAnnotationKey]; artifactJSON != "" {
		var arts []annotations.ArtifactOCIAnnotation
		if err := json.Unmarshal([]byte(artifactJSON), &arts); err != nil {
			return repository.OwnershipReferrer{}, fmt.Errorf("failed to parse %s on ownership referrer %s: %w", annotations.ArtifactAnnotationKey, desc.Digest, err)
		}
		if len(arts) > 0 {
			owner.Artifact = arts[0].Identity
		}
	}
	return owner, nil
}
...
func (repo *Repository) DownloadResourceStream(ctx context.Context, res *descriptor.Resource) (ocistream.ResourceStream, error) {
	...
	// NOTE: ownership referrers (ADR 0016) are intentionally NOT fetched here.
	// DownloadResource is also used during by-value construction
	// (constructor.processResourceByValue), where pulling an external image's
	// pre-existing referrers into the downloaded layout would break the
	// subsequent AddLocalResource pack. By-value resources carry their referrers
	// via the local-blob path instead (see getLocalBlobFromIndexOrManifest). A
	// properly scoped trigger that distinguishes transfer-of-owned-artifact from
	// ingest-of-external-image is needed before this can fetch them here.
	return stream, nil
}
