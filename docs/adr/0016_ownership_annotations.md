...
A referrer is created only for resources that **opt in** via `options.ownershipPolicy: Always` (the default is `Never`); resources that do not opt in, and all sources, never get one. The opt-in is independent of `relation` — both `local` and `external` resources may opt in. There are **two creation paths**, selected by how the resource is stored:

| `ocm add cv` case | Code entry point | Subject / where the referrer lands |
| --- | --- | --- |
| opted in, **by value** (`input:` method or `access` + `copyPolicy: byValue`), packed as an OCI manifest | `AddLocalResource` → `uploadAndUpdateLocalArtifact` → `pack` | the uploaded manifest, in the **component repository** |
| opted in, **by reference** (`access.imageReference`, kept by reference) | `attachOwnershipReferrer` → `AddOwnershipReferrer` | the referenced image, in its **hosting registry** (image untouched) |

**Path A — by value (referrer pushed into the component repository).** The resource blob is uploaded through [`repository.go`](https://github.com/open-component-model/open-component-model/blob/main/bindings/go/oci/repository.go) `uploadAndUpdateLocalArtifact`, which calls `configureOwnershipReferrer` to set `packOptions.Referrer = tar.ReferrerSource{ArtifactType: OwnershipArtifactType, CreateFunc: pack.OwnershipReferrer(resource, component, version)}` — `CreateFunc` is filled in only when the artifact is a `*descriptor.Resource` that opted in (`OwnershipPolicy == Always`). [`pack.go`](https://github.com/open-component-model/open-component-model/blob/main/bindings/go/oci/internal/pack/pack.go) `ResourceLocalBlobOCILayout` forwards that single `Referrer` to [`blob_io.go`](https://github.com/open-component-model/open-component-model/blob/main/bindings/go/oci/tar/blob_io.go) `CopyOCILayoutWithIndex`, which (absent an inbound referrer to copy) calls `CreateFunc` and pushes the result **as an additional `CopyGraph` root** alongside the resource. Note: this only applies to the **OCI-layout** pack path — a resource whose blob is a single OCI **layer** (a plain `file`/`dir` input) goes through `ResourceLocalBlobOCILayer`, has no manifest subject, and gets no referrer.

**Path B — by reference (referrer pushed into the image's own registry).** A resource kept by reference is never re-uploaded, so the referrer is attached next to the existing image. [`construct.go`](https://github.com/open-component-model/open-component-model/blob/main/bindings/go/constructor/construct.go) `processResource` calls `attachOwnershipReferrer` (only when the resource opted in via `OwnershipPolicy == Always`), which resolves credentials and invokes `ResourceRepository.AddOwnershipReferrer`. This is an **optional capability**: a resource repository advertises it via [`OwnershipReferrerAttacher`](https://github.com/open-component-model/open-component-model/blob/main/bindings/go/constructor/interface.go) (defined in the constructor layer); the constructor type-asserts for it and treats its absence as a no-op, so repositories that cannot host referrers (classic Helm HTTP repos, out-of-process plugins) implement nothing. For OCI, `oci.Repository.AddOwnershipReferrer` resolves the referenced image in its hosting registry (`StoreForReference(imageReference)` + `Resolve(tag)`), builds the referrer against that subject, and `Push`es it into the **same registry as the image**. The image bytes are unchanged, so OCI-level signatures stay valid.

> **Status — wired through the CLI.** `ocm add cv` resolves the OCI resource repository and, when it implements `repository.OwnershipReferrerRepository`, wraps it as `constructorPluginWithOwnershipReferrer` (a `constructor.OwnershipReferrerAttacher`), so Path B runs end to end via the command. Resource repositories that cannot host referrers (out-of-process plugins, classic Helm HTTP repos) do not implement the capability, and the attach is a no-op for them.
...
- **Target — copy, don't re-create.** `AddLocalResource` → `uploadAndUpdateLocalArtifact` → `configureOwnershipReferrer` sets `packOptions.Referrer.ArtifactType = OwnershipArtifactType` for every resource (the copy selector is set unconditionally; `CreateFunc` is added only when the resource opted in (`OwnershipPolicy == Always`)). In `CopyOCILayoutWithIndex`, an inbound referrer of `ReferrerSource.ArtifactType` found in the layout index **replaces creation**: `existingReferrers` wraps it as the `ReferrersFunc`, so it is carried over verbatim (instead of `CreateFunc` building a new one) and copied as its own root via `tar.CopyReferrerRoots` — created and copied referrers share one path.
- **OCI-image-by-reference transfer (target-side only — source discovery is a follow-up).** When a resource is transferred as an OCI image (`UploadResource` → `uploadOCIImage`), referrers that travelled inside the layout are held aside by `partitionOwnershipReferrers` (so the main artifact is still selected correctly) and copied with `uploadOwnershipReferrers`. The target-side copy machinery is wired, but the **source side does not yet discover** a by-reference image's referrers: `DownloadResourceStream` skips the Referrers-API lookup (`TODO(ocm-project#1025)` in `repository.go`) because that same download path is also used to ingest external images during by-value construction, where pulling pre-existing referrers would corrupt the pack. So a by-reference image's ownership referrer is **not yet carried on `ocm transfer`** — a follow-up, like the CTF directions below. By-value resources are unaffected: their referrer is discovered and packed by the local-blob path above.
...
> **Status — only OCI→OCI is implemented and tested.** Issue #1025 also calls for OCI→CTF and CTF→OCI. The copy mechanism rides inside the OCI layout, so a referrer is expected to travel to a CTF target, but CTF exposes no live Referrers API — so discovery on the CTF side differs, and neither CTF direction is implemented or covered by an integration test yet. `Test_Integration_AssetToOwner` exercises OCI→OCI only; the other two directions are a follow-up.
