...
> **Status — not yet wired through the CLI.** The only resource repository the CLI hands the constructor is the in-process plugin bridge (`constructorPlugin`), which does not implement `OwnershipReferrerAttacher`, so the type-assertion fails and Path B is currently a no-op via `ocm add cv`. The OCI implementation (`oci.Repository.AddOwnershipReferrer`) and the constructor call site (`attachOwnershipReferrer`) exist and are unit-tested; forwarding the capability through the bridge is a follow-up. Path A (by value) and Transfer are unaffected.
...
- **Target — copy, don't re-create.** `AddLocalResource` → `uploadAndUpdateLocalArtifact` → `configureOwnershipReferrer` sets `packOptions.Referrer.ArtifactType = OwnershipArtifactType` for every resource (the copy selector is set regardless of relation; `CreateFunc` is added only for `relation: local`). In `CopyOCILayoutWithIndex`, an inbound referrer of `ReferrerSource.ArtifactType` found in the layout index **replaces creation**: `existingReferrers` wraps it as the `ReferrersFunc`, so it is carried over verbatim (instead of `CreateFunc` building a new one) and copied as its own root via `tar.CopyReferrerRoots` — created and copied referrers share one path.
